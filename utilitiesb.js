...
function createConfigButton() {
  ...
  configButton.style.fontSize = (window.innerWidth * 0.03) + 'px';
...
function createConfigPanel() {
  ...
  panel.style.fontSize = (window.innerWidth * 0.03) + 'px';
  ...
  closeBtn.fontSize = (window.innerWidth * 0.03) + 'px';
...
Object.keys(resMap).forEach(level => {
  ...
  btn.style.fontSize = (window.innerWidth * 0.03) + 'px';
...
muteBtn.innerText = mainSong.muted ? "Unmute" : "Mute";
muteBtn.style.flex = '1';
muteBtn.style.padding = '8px';
muteBtn.style.borderRadius = '8px';
muteBtn.style.border = 'none';
muteBtn.style.cursor = 'pointer';
muteBtn.style.fontSize = (window.innerWidth * 0.03) + 'px';
muteBtn.style.fontWeight = 'bold';
muteBtn.style.background = mainSong.muted ? '' : 'red';
muteBtn.style.color = mainSong.muted ? '' : 'white';
...
function createExtraButton() {
  ...
  extraButton.style.fontSize = (window.innerWidth * 0.03) + 'px';
...
function updateLightViewMatrix() {
  ...
  let  rx = createRotationMatrix({ x: 1, y: 0, z: 0 }, camera.rotation.x-radian*25);
  let  ry = createRotationMatrix({ x: 0, y: -1, z: 0 }, camera.rotation.y);
  let  rz = createRotationMatrix({ x: 0, y: 0, z: 1 }, camera.rotation.z);
...
function updateLightViewMatrix() {
  ...
  let light = {
    position: { x: camera.position.x, y: 20, z: camera.position.z-30},
    rotation: { x:0, y: 0, z: 0 }
  };
  
  let camPosMatrix = [
    [1, 0, 0, -light.position.x],
    [0, 1, 0, -light.position.y],
    [0, 0, 1, -light.position.z],
    [0, 0, 0, 1]
  ];
  
  let rx = createRotationMatrix({ x: 1, y: 0, z: 0 }, light.rotation.x);
  let ry = createRotationMatrix({ x: 0, y: -1, z: 0 }, light.rotation.y);
  let rz = createRotationMatrix({ x: 0, y: 0, z: 1 }, light.rotation.z);
  
  let rotationMatrix = matrixMultiply(rx, ry, rz);
  
  light.forward = matrixTimesVector({ x: 0, y: 0, z: -1 }, rotationMatrix);
  light.up = matrixTimesVector({ x: 0, y: 1, z: 0}, rotationMatrix);
  light.right = matrixTimesVector({ x: 1, y: 0, z: 0 }, rotationMatrix);
  
  return matrixMultiply(camPosMatrix, rotationMatrix);
}
*/
//-----------------------//
...
function lookAt(eye, target, up) {
  // Calcula vectores
  let zAxis = normV(vectorSub(eye, target)); // hacia atrás
  let xAxis = normV(cross(up, zAxis)); // derecha
  let yAxis = cross(zAxis, xAxis); // arriba
  
  // Matriz en row-major
  return [
    [xAxis.x, xAxis.y, xAxis.z, -dotP(xAxis, eye)],
    [yAxis.x, yAxis.y, yAxis.z, -dotP(yAxis, eye)],
    [zAxis.x, zAxis.y, zAxis.z, -dotP(zAxis, eye)],
    [0, 0, 0, 1]
  ];
}
